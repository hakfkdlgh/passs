...
def generate_pass(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        text=f'your password is ready:\n`{final_dic_and_pass(settingg())}`',parse_mode=telegram.ParseMode.MARKDOWN)
...
def show_accounts(update: Update, context: CallbackContext):
    ...
    try:
        for key in account[user_id]:
            ...
            password: `{account[user_id][key][-1]}`''',parse_mode=telegram.ParseMode.MARKDOWN)
    except:
        update.message.reply_text(
            text="an error occured, or you have not set any password yet", reply_to_message_id=update.message.message_id)
...
def cancel_acc(update: Update, context: CallbackContext) -> int:
    ...
    return -1
def del_acc_command(update:Update, context:CallbackContext):
    user_id = update.message.chat_id
    try:
        update.message.reply_text(text='these are the names of your saved accounts',reply_to_message_id=update.message.message_id)
        for key in account[user_id]:
            update.message.reply_text(text=f'<pre>{key}</pre>',parse_mode=telegram.ParseMode.HTML)
        update.message.reply_text(text='''to delete  account use these command
    /del <account1> <account2> ...
    for exmple /del google clash''') 
    except:
        update.message.reply_text(text='you havent save any account\ntry /add_acc to add some')
def del_acc(update:Update, context:CallbackContext):
    user_id = update.message.chat_id
    try:
